...
export default function PatchesAdmin() {
  ...
  const [form, setForm] = useState({ version: "", date: "", size: "", supernovaLink: "", bigbangLink: "", supernovaRomLink: "", bigbangRomLink: "", supernovaRomSize: "", bigbangRomSize: "", changelog: "", isLatest: false });

  useEffect(() => { load(); }, []);

  const load = async () => setPatches(await api.getPatches());

  const resetForm = () => setForm({ version: "", date: "", size: "", supernovaLink: "", bigbangLink: "", supernovaRomLink: "", bigbangRomLink: "", supernovaRomSize: "", bigbangRomSize: "", changelog: "", isLatest: false });

  const startEdit = (p?: PatchVersion) => {
    if (p) {
      setEditing(p.id);
      setForm({ version: p.version, date: p.date, size: p.size, supernovaLink: p.supernovaLink || "", bigbangLink: p.bigbangLink || "", supernovaRomLink: p.supernovaRomLink || "", bigbangRomLink: p.bigbangRomLink || "", supernovaRomSize: p.supernovaRomSize || "", bigbangRomSize: p.bigbangRomSize || "", changelog: p.changelog.join("\n"), isLatest: p.isLatest });
    } else {
      setEditing("new");
      resetForm();
    }
        ...
        <Card className="bg-slate-900 border-white/10">
          <CardContent className="p-4 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              ...
              <Label className="flex items-center gap-2 text-sm text-slate-300"><Switch checked={form.isLatest} onCheckedChange={(v) => setForm({ ...form, isLatest: v })} /> Dernière version</Label>
              ...
              <Input placeholder="Lien ROM Supernova" value={form.supernovaRomLink} onChange={(e) => setForm({ ...form, supernovaRomLink: e.target.value })} className="bg-slate-800 border-white/10 text-white" />
              ...
              <Input placeholder="Lien ROM Big Bang" value={form.bigbangRomLink} onChange={(e) => setForm({ ...form, bigbangRomLink: e.target.value })} className="bg-slate-800 border-white/10 text-white" />
            ...
            <Textarea placeholder="Changelog (une ligne par changement)" value={form.changelog} onChange={(e) => setForm({ ...form, changelog: e.target.value })} className="bg-slate-800 border-white/10 text-white min-h-24" />
