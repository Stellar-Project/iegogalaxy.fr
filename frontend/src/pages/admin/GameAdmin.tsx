...
import { Plus, Pencil, Trash2, Check, X, Gamepad2 } from "lucide-react";

export default function GameAdmin() {
  const [games, setGames] = useState<Game[]>([]);
  const [editing, setEditing] = useState<string | null>(null);
  const [form, setForm] = useState({ slug: "", name: "", description: "", imageUrl: "", status: "0%", releaseDate: "", sortOrder: 0, published: false });
  ...
  const startEdit = (g?: Game) => {
    if (g) { setEditing(g.id); setForm({ slug: g.slug, name: g.name, description: g.description, imageUrl: g.imageUrl || "", status: g.status, releaseDate: g.releaseDate || "", sortOrder: g.sortOrder, published: g.published }); }
    else { setEditing("new"); setForm({ slug: "", name: "", description: "", imageUrl: "", status: "0%", releaseDate: "", sortOrder: games.length, published: false }); }
  };

  const save = async () => {
    if (editing === "new") await api.createGame(form);
    else if (editing) await api.updateGame(editing, form);
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <Button onClick={() => startEdit()} size="sm" className="bg-blue-600 hover:bg-blue-500 text-white"><Plus size={16} className="mr-1" /> Ajouter un jeu</Button>
      </div>
        ...
        <Card className="bg-slate-900 border-white/10">
          <CardContent className="p-4 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <Input placeholder="Nom du jeu" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value, slug: form.slug === form.name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || !form.slug ? e.target.value.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") : form.slug })} className="bg-slate-800 border-white/10 text-white" />
            ...
            <CardContent className="p-4 flex items-center justify-between">
              <div className="flex items-center gap-3 flex-1 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">{g.status} — {g.description.slice(0, 80)}</p>
                </div>
              </div>
